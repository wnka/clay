...
// On AWS Bedrock, Codex model IDs must be prefixed with "openai." (e.g.
// "openai.gpt-5.6-sol"). This is driven by the `codexBedrock` flag in the Clay
// daemon config; off Bedrock the model IDs stay bare.
function isCodexBedrock() {
  try {
    return require("../../config").isCodexBedrock();
  } catch (e) {
    return false;
  }
}
...
// On AWS Bedrock, Codex model IDs must be prefixed with "openai." (e.g.
// "openai.gpt-5.6-sol"). Upstream keeps a bare-string CODEX_MODELS list; we apply
// the prefix at the boundary (only when Codex targets Bedrock) so that list can
// stay byte-for-byte identical to upstream and keep merging cleanly. Idempotent
// — never double-prefixes, and a no-op on non-Bedrock setups.
function codexModelId(id) {
  if (!id || !isCodexBedrock()) return id;
  return id.indexOf("openai.") === 0 ? id : "openai." + id;
}

var DEFAULT_CODEX_MODEL = codexModelId("gpt-5.6-sol");
var FALLBACK_CODEX_MODELS = [
  { value: DEFAULT_CODEX_MODEL, displayName: "gpt-5.6-sol", supportsEffort: true, supportedEffortLevels: ["minimal", "low", "medium", "high"] },
];
...
function flattenEvent(notification, state) {
  ...
  if (method === "turn/completed") {
    ...
    resultModelUsage[state.model] = {
      contextWindow: state.lastContextWindow || state.modelContextWindow || null,
    };
  ...
  if (method === "thread/tokenUsage/updated") {
    ...
    if (tu) {
      var turnUsage = tu.lastTurn || tu.last || tu.total;
      if (turnUsage) {
      ...
      if (tu.modelContextWindow != null) {
        ...
        state.modelContextWindow = tu.modelContextWindow;
      }
    }
    return events;
  }
...
function createCodexAdapter(opts) {
  ...
  var _cachedModels = CODEX_MODELS.map(codexModelId);
  ...
  var adapter = {
    ...
    init: function(initOpts) {
      ...
      _initPromise = (async function() {
        ...
        _cachedModels = CODEX_MODELS.map(codexModelId);
        ...
        try {
          var modelListResult = await _appServer.send("model/list", {}, 30000);
          if (isCodexBedrock() && modelListResult && Array.isArray(modelListResult.data)) {
            modelListResult = {
              data: modelListResult.data.filter(function(model) {
                var id = model && (model.model || model.id);
                return typeof id === "string" && id.indexOf("openai.gpt-") === 0;
              }),
            };
          }
          var normalizedModels = normalizeCodexModelList(modelListResult);
          if (normalizedModels.models.length > 0) {
            _cachedModels = normalizedModels.models;
            _defaultModel = normalizedModels.defaultModel;
          }
        } catch (e) {
        ...
        console.log("[codex] App-server initialized, models: " + _cachedModels.map(codexModelValue).join(", "));
    ...
    supportedModels: function() {
      return Promise.resolve(_cachedModels);
    },
