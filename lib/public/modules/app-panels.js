...
var KNOWN_CONTEXT_WINDOWS = {
  ...
  "openai.gpt-5.5": 1048576,
  "openai.gpt-5.4": 1048576,
  "openai.gpt-5.3": 1048576,
  "openai.gpt-5.2": 1048576,
  "gpt-4.1": 1047576,
  "o3": 200000,
  "o4-mini": 200000,
};
...
function rebuildModelList() {
  ...
  // Picker visibility: always shown. Locked (non-clickable) after the
  // first message for GUI sessions — both Claude and Codex bind the model
  // at session/thread creation and cannot change it mid-conversation.
  // TUI sessions allow mid-thread model swaps so they stay unlocked.
  ...
  var lockedAfterFirstMessage = s.activeSessionMode === "gui"
    && !!s.sessionHasHistory;
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (lockedAfterFirstMessage) btn.classList.add("locked");
    btn.dataset.model = value;
    btn.textContent = label;
    if (lockedAfterFirstMessage) {
      btn.disabled = true;
      btn.title = "Model is locked after the first message. Start a new session to change it.";
    } else {
